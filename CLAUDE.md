...
#### Stamp Management
- `POST /api/v1/stamps/`: Purchase new postage stamps (records purchase time for propagation tracking). **Bounded** by `X402_MAX_STAMP_BZZ` per request and `STAMP_DAILY_BZZ_PER_CALLER` per caller per day — see below. Labels are sent to Bee as the `?label=` query parameter (a JSON body was ignored by Bee, so labels used to be dropped). Caller labels: at most 64 of `[A-Za-z0-9._-]` (checked before payment), not `recovered`, not starting with `paid-`/`pool-`/`synced-`; `""` means none. **Lost Bee response after payment (#400):** a paid purchase sends Bee a label unique to it: the caller's plus `-<12 hex>`, else `paid-<12 hex>`. If Bee gives no answer after settlement (a transport error or timeout, `SWARM_STAMP_PURCHASE_TIMEOUT_SECONDS`, default 120, or a 502/504 from a proxy), `find_purchased_batch` polls Bee's `/stamps` for up to `STAMP_PURCHASE_LOOKUP_SECONDS` (30) for exactly one batch with that unique label, depth and amount, created at or after the chain block noted before the purchase, and not registered to anyone; a failed poll counts as "not found yet". Bee's `recovered` batches are deliberately **not** matched (nothing ties one to this purchase rather than a concurrent one); those go to the refund path. Found → registered to the payer with `register_stamp(only_if_unowned=True)`, normal 201. Not found (or already owned) → `purchase_pending` audit event (tx, label, depth, amount, start block, payer) and `202 PURCHASE_PENDING`; a background task (held in `_PENDING_TASKS`) keeps looking for `STAMP_PURCHASE_BACKGROUND_LOOKUP_SECONDS` (900). Found → registered, `payment_delivered` (`late: true`), stored Idempotency-Key result replaced with the 201. Otherwise (not found, taken, an error of its own, or cancelled at shutdown) → `payment_failed` `delivery_after_settlement` and the stored key result becomes `500 DELIVERY_FAILED_AFTER_PAYMENT`. `register_stamp` logs an ERROR whenever a batch changes hands between non-pool owners. Ownership is registered as soon as the batch id is known; bookkeeping after it (spend budget, purchase time, metrics) cannot fail the response.
