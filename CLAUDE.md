...
#### Stamp Management
- `POST /api/v1/stamps/`: Purchase new postage stamps (records purchase time for propagation tracking). **Bounded** by `X402_MAX_STAMP_BZZ` per request and `STAMP_DAILY_BZZ_PER_CALLER` per caller per day — see below. Labels are sent to Bee as the `?label=` query parameter (a JSON body was ignored by Bee, so labels used to be dropped). **Lost Bee response after payment (#400):** a paid purchase always has a label (the caller's, else `paid-<hex>`). If Bee gives no answer (timeout `SWARM_STAMP_PURCHASE_TIMEOUT_SECONDS`, default 120) after settlement, `find_purchased_batch` looks in Bee's `/stamps` for up to `STAMP_PURCHASE_LOOKUP_SECONDS` (30) for exactly one unregistered batch with that label, depth and amount (or Bee's `recovered` label, created at or after the chain block when the purchase started). Found → registered to the payer, normal 201. Not found → `202 PURCHASE_PENDING` with `transaction`, `label` and a lookup hint; a background task keeps looking for `STAMP_PURCHASE_BACKGROUND_LOOKUP_SECONDS` (900), registers the batch, audits `payment_delivered` (`late: true`) and replaces the stored Idempotency-Key result with the 201, or logs `payment_failed` (`delivery_after_settlement`) if it never appears. The background task does not survive a restart. Ownership is registered as soon as the batch id is known; bookkeeping after it (spend budget, purchase time, metrics) cannot fail the response.
