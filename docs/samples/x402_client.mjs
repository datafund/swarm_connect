...
//   npm install x402@1 viem                                    # only for --paid
...
// refusing anything above MAX_USD. Exits non-zero on any failure.
...
async function call(url, makeInit) {
  ...
  if (res.status === 402) {
    const requirement = paymentRequirements(await res.json())[0];
